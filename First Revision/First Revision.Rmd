...
```{r import and clean}
reduced <- read_csv("Data with Depression.csv")
...
interactions::sim_slopes(model = arousalinteract, pred = alexithymia , modx = condition, confint = TRUE, digits = getOption("jtools-digits",
  default = 3))
```
## Plot model
```{r plot arousal}
...
interactions::sim_slopes(model = valenceinteract, pred = alexithymia , modx = condition, confint = TRUE, digits = getOption("jtools-digits",
  default = 3), johnson_neyman = FALSE)
```
##Plot Model
```{r valence plot, message = FALSE}
valenceplot <- interactions::interact_plot(model = valenceinteract, 
                                           pred = alexithymia, 
                                           modx = condition,
                                           modx.values = c("Tender",
                                                           "Sad", 
                                                           "Happy",
                                                           "Fearful",
                                                           "Angry"),
                                           interval = TRUE, 
                                           legend.main = c("Target Emotion"))                    +
  theme_classic() + 
...
diyarousal <- ggplot(rr.long.arousal(x = ))
...
## Save plot
```{r}
ggplot2::ggsave(filename = "valence.jpg", 
       plot = valenceplot, 
       width = 9,
       height = 7,
       dpi = 320)

ggplot2::ggsave(filename = "arousal.jpg", 
       plot = arousalplot, 
       width = 9,
       height = 7,
       dpi = 320)
